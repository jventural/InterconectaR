#' Plot two networks side by side with R2 progress rings
#'
#' Creates a faceted ggplot comparing two group networks (e.g., female vs male)
#' with nodes colored by domain, R2 progress rings, gradient-colored edges,
#' and optional abbreviated labels with a legend mapping.
#'
#' @param mat1 Adjacency matrix for the first group.
#' @param mat2 Adjacency matrix for the second group.
#' @param r2_1 Numeric vector of R2 values for the first group.
#' @param r2_2 Numeric vector of R2 values for the second group.
#' @param group_names Character vector of length 2 with facet labels (default: c("Femenino","Masculino")).
#' @param labels Character vector of node labels (default: colnames of mat1).
...
#' @importFrom ggplot2 ggplot aes geom_curve geom_segment scale_size geom_point scale_color_manual guides guide_legend scale_fill_manual geom_text geom_label coord_equal theme_void theme labs element_text facet_wrap annotation_custom
...
#' \dontrun{
#' plot_net_group(
#'   mat1 = net_fem$graph, mat2 = net_masc$graph,
#'   r2_1 = r2_fem, r2_2 = r2_masc,
#'   labels = colnames(net_fem$graph),
#'   short_labels = c("AE","HZ","CT"),
#'   full_labels  = c("Agotamiento Emocional","Hartazgo","Contraste")
#' )
#' }
plot_net_group <- function(
    mat1, mat2,
    r2_1, r2_2,
    group_names = c("Femenino", "Masculino"),
  ...
  # --- Layout comun ----------------------------------------------------------
  L <- qgraph::averageLayout(list(mat1, mat2))
  nodes_base <- dplyr::tibble(node = seq_along(labels),
                               label = display_labels,
                               x = L[,1], y = L[,2])

  if (!is.null(groups_list)) {
    dom_df <- purrr::imap_dfr(groups_list, ~dplyr::tibble(node = .x, domain = .y))
    nodes_base <- nodes_base %>% dplyr::left_join(dom_df, by = "node")
  } else {
    nodes_base <- nodes_base %>% dplyr::mutate(domain = "Nodos")
    if (!"Nodos" %in% names(group_cols)) group_cols <- c(group_cols, Nodos="#95A5A6")
  }

  clamp01 <- function(x) pmin(pmax(x, 0), 1)
  nodes_long <- dplyr::bind_rows(
    nodes_base %>% dplyr::mutate(Group = group_names[1], R2 = clamp01(r2_1)),
    nodes_base %>% dplyr::mutate(Group = group_names[2], R2 = clamp01(r2_2))
  )
  ...
  r2_labels <- nodes_long %>%
    dplyr::transmute(Group, x = x + r2_shift$dx, y = y + r2_shift$dy,
                     label = paste0("R\u00b2=", format(round(R2, R2_digits),
                                                       nsmall = R2_digits)))

  # --- Aristas con clipping --------------------------------------------------
  nodes_xy <- nodes_base %>% dplyr::select(node, x, y)
  clip_r <- ring_r + 0.005

  to_edges <- function(A, group_name){
    idx <- which(upper.tri(A), arr.ind = TRUE)
    e <- dplyr::tibble(i=idx[,1], j=idx[,2], w=A[idx]) %>%
    ...
    if (nrow(e) == 0) return(e %>% dplyr::mutate(x=numeric(), y=numeric(),
    ...
    e <- e %>%
      ...
      dplyr::select(x, y, xend, yend, lx, ly, w, abs_w, sign, Group)

    e
  }

  edges <- dplyr::bind_rows(
    to_edges(mat1, group_names[1]),
    to_edges(mat2, group_names[2])
  )
  ...
  # --- Datos de anillos ------------------------------------------------------
  ring_bg   <- nodes_long %>% dplyr::transmute(Group, x0=x, y0=y, r=ring_r)
  ring_prog <- nodes_long %>% dplyr::transmute(Group, x0=x, y0=y, r=ring_r,
                                               start=start_angle,
                                               end=start_angle + 2*pi*R2)

  pad <- ring_r + 0.12
  xr  <- range(nodes_base$x); yr <- range(nodes_base$y)
