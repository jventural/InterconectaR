#' Combine Network and Centrality Graphs
#'
...
#' @param layoutScale Layout scale vector.
#' @param vsize Node size.
#' @param threshold Minimum edge weight threshold.
#'
#' @export
...
combine_graphs_centrality  <- function(Figura1_Derecha, network, groups, error_Model,
  ...
  # Aplicar threshold a la matriz de adyacencia
  graph_filtered <- network$graph
  graph_filtered[abs(graph_filtered) < threshold] <- 0
